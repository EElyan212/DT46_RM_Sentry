...
# 3. 在新终端启动 decision
gnome-terminal --title="decision" -- bash -c "cd $WS_DIR && source install/setup.bash &&  ros2 run rm_application rm_decision --ros-args --params-file src/rm_application/config/decision_params_blue.yaml; exec bash"
